...
- 导入完成后智能引用记忆（`srmsave`）按需保存，通常不必保存：由于一般每次导入都是全量导入，因此如果保存了记忆文件，下次导入时会认为本次分配的空间已被占用，导致无法正确分配空间并写回地址，从而引发导入失败。除非你非常清楚自己在做什么，并且知道如何管理智能引用记忆，否则建议不要保存记忆文件。
...
for i = 1, #fileList do
    ...
    -- 导出文件
    ret, err = export("target:/" .. filePath, fileId)
    -- 如果 err 不为 nil，说明导出过程中发生了错误，且 ret 将是 nil
    if err ~= nil then
        log("错误：导出文件 " .. filePath .. ": " .. err)
    else
        log("完成: " .. filePath)
    end
end
...
4. **错误处理**：
   - `import` 会在失败时抛出异常
   - Mule 侧发生异常时，除了会记录日志，还会返回给 LUA 脚本一个错误码和错误信息，脚本可以据此进行错误处理。
...
字段可以包含约束条件来控制读写行为：
```xml
<struct name="ConstrainedData">>
    <field name="value" type="uint32">
        <constraint>lt:100</constraint>  <!-- 值必须小于 100 -->
    </field>
</struct>
```
约束条件支持以下类型：
...
只对诸如int一类的基本类型字段有效，且在导入时会进行验证。

##### 约束违反
对于无限长度数组，约束条件用于判断何时停止读取。当读取到一个元素时，如果该元素的值违反了约束条件，则停止读取，并且该元素*包含*在数组中，但是只到发生违反的元素位置。即：若有一个结构体的无限长度数组，第一个字段约束值不为 -1，第二个字段无约束。则当遇到第一个字段值为 -1 的元素时，停止读取，该元素的第一个字段包含在结果中，而第二个字段不包含在结果中。

对于其他类型，若导入/导出时字段值违反了约束条件，则会导致导入/导出失败，并抛出异常。
...
2. **导入时**：在可用空间中分配内存，写入数据并更新指针。并且，特别地，如果对象是字符串，保证每个字符串仅有一个实例，即如果多个字段引用了相同内容的字符串，则它们会共享同一个内存地址。
