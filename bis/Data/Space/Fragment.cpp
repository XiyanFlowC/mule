...
{
	if (size >= this->size) throw InvalidParameterException("size", "Cannot larger than this->size.", __FILE__, __LINE__);
...
{
	if (size > this->size) throw InvalidParameterException("size", "Cannot be larger than this->size.", __FILE__, __LINE__);
...
void Fragment::Merge(const Fragment &target)
{
	...
	if (!IsOverlapsWith(target)) throw InvalidParameterException("target", "Have no insertion with this fragment.", __FILE__, __LINE__);
